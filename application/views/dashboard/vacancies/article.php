            ...
            <div class="wrp">
                <div class="w-1 row jsfd">
                    ...
                    <?php if (array_key_exists('id', $article)) : ?>
                        <a target="_blank" class="btn" href="<?php echo base_url(); ?>vacancies/blog/<?= $article['slug'] ?>">Посмотреть статью</a>
                        <?php $this->load->view('widgets/_form-delete', array('delete_path' => 'vacancies/deleteArticle', 'slug' => $article['id'], 'type' => 'articles', 'disabled' => $permissions['vacancies_delete'])); ?>
                    <?php else: ?>
                ...
                <div class="w-1">
                    ...
                    if (isset($fields)) {
                        $this->load->view('widgets/_form', array(
                                'fields' => $fields,
                                'data' => $article,
                                'post' => base_url() . 'dashboard/vacancies/addArticle',
                                'ajax' => 'vacancies/updateArticle',
                            )
                        );
                    }
                    ?>
    ...
    <section id="MediaUpload">
        <div>
            ...
            <div class="wrp">
                ...
                $files = get_filenames('./media/vacancies_articles/' . $article['id'] . '/gallery/');
                $imgs = [];
                foreach ($files as $f) {
